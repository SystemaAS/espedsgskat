    ...
    jq(function() {
  	...
  	jq(function() {
  	  	jq('#importInvoicesButton').click(function() {
  	  		window.open('skatimport_edit_childwindow_external_invoices.do?avd=' + jq("#avd").val() + "&opd=" + jq("#opd").val(), 'importInvoicesWin','top=120px,left=100px,height=650px,width=800px,scrollbars=no,status=no,location=no');
  	  	});
  	  	/*
            ...
            buttons: [ 
                {
    			 ...
    			 click: function(){
    		            ...
    		            //do delete
    		            jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
    		            window.location = "skatimport_edit_invoice.do?action=doDelete" + "&avd=" + avd + "&opd=" + opd + "&sign=" + sign + "&status=" + status + "&fak=" + fak;
    			 	 }
    		 	 },
     	 		{
    		 	 id: "dialogCancel",
  	  		  ...
  	  		  success: function(data) {
  		  		  ...
  		  		  if(len>0){
  				  	...
  				  	if(data.indexOf('[OK') == 0) {
  					  	...
  					  	//reload
  					  	window.location = "skatimport_edit_invoice.do?action=doFetch&avd=" + avd + "&opd=" + opd + "&sign=" +  sign;
  				  	}
  				  	//unblock
  				  	jq.unblockUI();
  		  		  }
