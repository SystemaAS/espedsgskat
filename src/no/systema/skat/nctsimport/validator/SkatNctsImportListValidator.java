 ...
 *
 */
public class SkatNctsImportListValidator implements Validator {

	/**
	 * 
	 */
	public boolean supports(Class clazz) {
		return SearchFilterSkatNctsImportTopicList.class.isAssignableFrom(clazz); 
	}
	...
	public void validate(Object obj, Errors errors) { 
		SearchFilterSkatNctsImportTopicList record = (SearchFilterSkatNctsImportTopicList)obj;
		...
		if(record!=null){
			...
			if( !this.valueExists(record.getAvd()) && !this.valueExists(record.getSign()) ){
				if( this.valueExists(record.getOpd()) ||  this.valueExists(record.getStatus()) || 
					...
					ValidationUtils.rejectIfEmptyOrWhitespace(errors, "avd", "systema.skat.ncts.import.list.error.null.avdOrSign"); 
				}
			}
