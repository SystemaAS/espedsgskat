...
/**
 * SKAT-NCTS Export Header Logging Controller 
 * 
...
@Controller
@Scope("session")
public class SkatNctsExportHeaderLoggingController {
	
	private static final Logger logger = LoggerFactory.getLogger(SkatNctsExportHeaderLoggingController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="skatnctsexport_logging.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("skatnctsexport_logging");
		logger.info("Method: doShowList [RequestMapping-->skatnctsexport_logging.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = SkatNctsExportUrlDataStore.NCTS_EXPORT_BASE_LOG_LIST_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonSkatNctsExportSpecificTopicLoggingContainer container = this.skatNctsExportSpecificTopicService.getNctsExportSpecificTopicLoggingContainer(jsonPayload);
	 ...
	 * @return
	 */
	@RequestMapping(value="skatncts_export_renderEdifact.do", method={ RequestMethod.GET })
	public ModelAndView doSkatNctsExportRenderEdifact(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doSkatNctsExportRenderEdifact...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
	...
	@RequestMapping(value="skatncts_export_renderXml.do", method={ RequestMethod.GET })
	public ModelAndView doSkatNctsExportRenderXml(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doSkatNctsExportRenderXml...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			if(filePath!=null && !"".equals(filePath)){
        		...
        		if(!new IPAddressValidator().isValidAbsoluteFilePathFor_RenderFile(absoluteFilePath)){
	                ...
	                //--> with browser dialogbox: response.setHeader ("Content-disposition", "attachment; filename=\"edifactPayload.txt\"");
	                response.setHeader ("Content-disposition", "filename=\"payload.xml\"");
	 ...
	 * @return
	 */
	@RequestMapping(value="skatncts_export_renderLargeText.do",  method= RequestMethod.GET)
	public ModelAndView doSkatNctsExportRenderLargeText(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		logger.info("Inside doSkatNctsExportRenderLargeText...");
		...
		if(appUser==null){
			...
			//get BASE URL = RPG-PROGRAM
            //---------------------------
			String BASE_URL = SkatNctsExportUrlDataStore.NCTS_EXPORT_BASE_LOG_LARGE_TEXT_FOR_SPECIFIC_TOPIC_URL;
			//url params
	...
	private String setLargeTextInDomainObject(String jsonPayload){
		...
		JsonSkatNctsExportSpecificTopicLoggingLargeTextContainer container = this.skatNctsExportSpecificTopicService.getNctsExportSpecificTopicLoggingLargeTextContainer(jsonPayload);
	 ...
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonSkatNctsExportSpecificTopicLoggingRecord record){
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsExportSpecificTopicLoggingContainer container){
	 ...
	 * @param jsonNctsExportSpecificTopicRecord
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsExportSpecificTopicLoggingRecord record){
		//SET HEADER RECORDS  (from RPG)
		model.put(SkatConstants.DOMAIN_RECORD, record);
	}
	/**
