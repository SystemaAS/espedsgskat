...
/**
 * 
 * SKAT-NCTS Import Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class SkatNctsImportHeaderArchiveController {
	 ...
	 * @return
	 */
	@RequestMapping(value="skatnctsimport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("skatnctsimport_archive");
		logger.info("Method: doShowList [RequestMapping-->skatnctsimport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = SkatNctsImportUrlDataStore.NCTS_IMPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonSkatNctsImportSpecificTopicArchiveContainer jsonNctsImportSpecificTopicArchiveContainer = this.skatNctsImportSpecificTopicService.getNctsImportSpecificTopicArchiveContainer(jsonPayload);
				...
				successView.addObject(SkatConstants.DOMAIN_LIST,jsonNctsImportSpecificTopicArchiveContainer.getArchiveElements());
	 ...
	 * @return
	 */
	@RequestMapping(value="skatncts_import_renderArchive.do", method={ RequestMethod.GET })
	public ModelAndView doSkatNctsImportRenderArchive(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doSkatNctsImportRenderArchive...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
	 ...
	 * @param record
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonSkatNctsImportSpecificTopicArchiveRecord record){
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsImportSpecificTopicArchiveContainer container){
	 ...
	 * @param record
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsImportSpecificTopicArchiveRecord record){
		//SET HEADER RECORDS  (from RPG)
		model.put(SkatConstants.DOMAIN_RECORD, record);
	}
	/**
